...
// ── Scanning tips shown in the upload zone ────────────────────────────────────
const TIPS = [
      ...
      <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <motion.div className="relative w-full max-w-lg rounded-2xl overflow-hidden z-10"
                    ...
                    {['top-0 left-0', 'top-0 right-0 rotate-90', 'bottom-0 left-0 -rotate-90', 'bottom-0 right-0 rotate-180'].map((cls, i) => (
                        <div key={i} className={`absolute ${cls} w-8 h-8`}>
                          <svg viewBox="0 0 32 32" fill="none">
                            <path d="M0 12 L0 0 L12 0" stroke="rgba(0,229,255,0.8)" strokeWidth="2.5"/>
                          </svg>
                        </div>
                    ))}
                    <p className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-xs whitespace-nowrap"
                       style={{ color: 'rgba(0,229,255,0.6)' }}>Position lesion in frame</p>
                  </div>
                </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between px-6 py-4"
            ...
            {!preview ? (
                <>
                  <button onClick={flip} className="w-10 h-10 rounded-xl flex items-center justify-center"
                          style={{ background: 'rgba(255,255,255,0.08)', color: '#fff' }}>
                    <RotateCcw size={18}/>
                  </button>
                  <button onClick={capture} disabled={!!error}
                          className="w-16 h-16 rounded-full flex items-center justify-center transition-transform hover:scale-105 disabled:opacity-40"
                          style={{ background: 'rgba(0,229,255,0.9)', boxShadow: '0 0 24px rgba(0,229,255,0.4)' }}>
                    <div className="w-12 h-12 rounded-full border-2 border-[#070d1a]"/>
                  </button>
                  <div className="w-10"/>
                </>
            ) : (
                <>
                  <button onClick={retake} className="px-4 py-2 rounded-xl text-sm font-semibold"
                          style={{ background: 'rgba(255,255,255,0.1)', color: '#fff' }}>
                    Retake
                  </button>
                  <button onClick={confirm}
                          className="px-5 py-2 rounded-xl text-sm font-bold"
                          style={{ background: '#00e5ff', color: '#070d1a' }}>
                    Use this photo ✓
                  </button>
                </>
            )}
...
const TipCard: React.FC = () => {
  ...
  return (
      <motion.div className="rounded-xl px-4 py-3 flex items-start gap-3"
                  style={{ background: 'var(--accent-dim)', border: '1px solid rgba(0,229,255,0.2)' }}>
        ...
        <AnimatePresence mode="wait">
          <motion.p key={idx} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                    ...
                    className="text-xs leading-relaxed" style={{ color: 'var(--tx2)' }}>
            <strong style={{ color: 'var(--accent)' }}>Tip {idx + 1}/{TIPS.length}:</strong>{' '}
            {TIPS[idx].text}
          </motion.p>
        </AnimatePresence>
      </motion.div>
  ...
  const analyzeFile = useCallback(async (selectedFile: File) => {
    ...
    try {
      ...
      try {
        ...
        prediction = {
          disease: 'Pending AI Analysis', confidence: 0,
          description: 'The AI backend is currently offline. Your image was saved and will be available in History.',
          suggestions: ['Check back when the AI backend is online.'],
        };
      }
      ...
      <div className="max-w-4xl mx-auto space-y-6 pb-8">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--tx2)' }}>
            Upload or take a photo of a skin area to start your AI analysis.
          </p>
          {isGuest && (
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--surface)', border: '1px solid var(--br)' }}>
          <AnimatePresence mode="wait">
            ...
            {/* Drop zone */}
            {!file && (
                <motion.div key="drop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.98 }}
                  ...
                  <div {...getRootProps()}
                    ...
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 transition-colors"
                         style={{ background: isDragActive ? 'var(--accent)' : 'var(--accent-dim)',
                  ...
                  {cameraSupported && (
                      <button onClick={() => setShowCamera(true)}
                              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold transition-all card-hover"
                              style={{ background: 'var(--surface2)', color: 'var(--tx2)', border: '1px solid var(--br)' }}
                              onMouseEnter={e => { e.currentTarget.style.color = 'var(--accent)'; e.currentTarget.style.borderColor = 'var(--accent)'; }}
                              onMouseLeave={e => { e.currentTarget.style.color = 'var(--tx2)'; e.currentTarget.style.borderColor = 'var(--br)'; }}>
                      ...
                      <div className="relative rounded-2xl overflow-hidden aspect-square"
                        ...
                        {!result && (
                            <button onClick={(e) => { e.stopPropagation(); reset(); }}
                                    className="absolute top-2.5 right-2.5 w-7 h-7 rounded-full flex items-center justify-center text-white"
                        ...
                        {loading && (
                            <div className="absolute inset-0 flex items-center justify-center"
                              ...
                              <div className="pring w-16 h-16 rounded-full border-2" style={{ borderColor: 'rgba(0,229,255,0.6)' }}/>
                            </div>
                        )}
                      </div>
                      ...
                      <AnimatePresence mode="wait">
                                    ...
                                    <div key={s} className="flex items-center gap-3 py-2 px-3 rounded-xl transition-all"
                                      ...
                                      <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                                        ...
                                        {i < stepIdx
                                            ...
                                            : i === stepIdx
                                                ? <span className="spin w-2.5 h-2.5 border-2 rounded-full block" style={{ borderColor: 'var(--br2)', borderTopColor: 'var(--accent)' }}/>
                                                : <span className="w-1.5 h-1.5 rounded-full block" style={{ background: 'var(--tx3)' }}/>}
                                      </div>
                                      <span className="text-xs font-medium"
                                            style={{ color: i === stepIdx ? 'var(--accent)' : i < stepIdx ? 'var(--tx2)' : 'var(--tx3)' }}>
                                {s}
                              </span>
                                    </div>
                                ))}
                        ...
                        {result && !loading && (
                            <motion.div key="result" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                                ...
                                <div className="flex items-center justify-between mb-2">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <div className="relative group">
                                      ...
                                      <div className="hidden group-hover:block absolute bottom-5 left-1/2 -translate-x-1/2 w-48 px-2.5 py-2 rounded-xl text-xs z-10"
                                           style={{ background: 'var(--surface)', border: '1px solid var(--br)', color: 'var(--tx2)',
                                             boxShadow: '0 8px 24px rgba(0,0,0,0.3)' }}>
                                        How confident the CNN model is in its prediction. &ge;80% is high confidence.
                                      </div>
                                    </div>
                                  </div>
                                  <span className="text-lg font-extrabold"
                                        style={{ color: confColor(confidenceDisplay), fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
                              {confidenceDisplay}%
                            </span>
                                </div>
                                <div className="h-2 rounded-full overflow-hidden" style={{ background: 'var(--br2)' }}>
                                  <motion.div className="h-full rounded-full"
                                              style={{ background: `linear-gradient(90deg, ${confColor(result.confidence)}77, ${confColor(result.confidence)})`, width: `${confidenceDisplay}%` }}/>
                                </div>
                              </div>
                                    ...
                                    <ul className="space-y-1.5">
                                      {result.suggestions.map((s: string, i: number) => (
                                          <li key={i} className="flex items-start gap-2 text-sm" style={{ color: 'var(--tx2)' }}>
                                            <ChevronRight size={14} className="mt-0.5 flex-shrink-0" style={{ color: 'var(--accent)' }}/>{s}
                                          </li>
                                      ))}
                                    </ul>
                                  </div>
                              ...
                              <button onClick={reset} className="btn-ghost w-full py-2.5 rounded-xl text-sm mt-2">
                                ↩ Scan another image
                              </button>
                            </motion.div>
                        )}
